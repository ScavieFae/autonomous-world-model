...
2. `docs/run-cards/` — individual experiment records (YAML frontmatter has status, RC, built_on)
3. `docs/decisions/` — rejected hypotheses with Director reasoning (important for "has this been considered?")
4. `docs/RESEARCH-LOG.md` — chronological findings and open items
5. `docs/experiments/index.md` — generated summary with experiment tree and RC leaderboard
...
**"What works for Y?"** — Report hit rates. "3/3 experiments with Self-Forcing improved RC. 1/1 with horizon weighting regressed." State the evidence, not an opinion.
...
- **Cite experiment IDs and numbers.** Not "Self-Forcing helped" but "e018a: Self-Forcing, RC=6.26 vs prior best 6.77 (7.5% improvement)."
- **Report hit rates, not conclusions.** "Improved in 3/3" is evidence. "This works" is editorial.
- **Flag uncited assumptions.** If a claim about what works or doesn't work has no experiment ID backing it, flag it. "This belief has no experimental evidence in our records. It may come from general ML intuition. Consider testing it."
...
- **Distinguish explored from exhausted.** One failure doesn't close an axis. "Absolute x caused oscillation (1/1 test)" is one data point, not a law. Report the evidence density alongside the result.
- **Check for duplicate IDs.** Some experiment IDs have multiple cards (e.g., e018b). Read both and clarify which you're referencing.
...
If the question touches training code specifics, read the relevant experiment YAML config in `experiments/` and the trainer code in `training/`.
