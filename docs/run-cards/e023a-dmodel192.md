...
| Metric | E018c (baseline) | E023a (d_model=192) | Delta |
|--------|-----------------|---------------------|-------|
| **rollout_coherence** | **6.03** | **6.065** | +0.58% |
| change_acc | 62.3% | 54.7% | -7.6pp |
| pos_mae | 0.824 | 0.845 | +2.5% |
| sf_loss | ~0.37 | 0.536 | +45% |
| total_loss | — | 0.515 | — |

Runtime: 11452s (~3.2hr) on L4. Cost: ~$2.56.
...
RC is within noise (6.065 vs 6.03, ±1%) but TF metrics show clear underfitting: change_acc -7.6pp, SF loss +45%. The 3.36× smaller model does NOT generalize better at 1.9K data. It trades one error regime for another (different position drift pattern averages to similar 20-horizon RC).

**Finding:** d_model=384 is not over-parameterized at 1.9K data. Halving width confirmed underfitting (1/1 experiments). Next: test d_model=768 to determine if model is capacity-constrained.
